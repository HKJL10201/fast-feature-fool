...
def caffenet(input):
    ...
    weights_path = os.path.join('weights', 'caffenet.npy')
    if not os.path.isfile(weights_path):
        print('Downloading CaffeNet weights...')
        urlretrieve ("https://www.dropbox.com/s/8evo8ydl1jl03z4/caffenet.npy?raw=1", os.path.join('weights', 'caffenet.npy'))
    net = np.load('weights/caffenet.npy',allow_pickle=True,encoding="latin1").item()
    weights = {}
    biases = {}
